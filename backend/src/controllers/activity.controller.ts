...
export async function create(req: AuthRequest, res: Response) {
  try {
    ...
    const activity = await activityService.createActivity(parsed.data, req.user!.id);
...
export async function update(req: AuthRequest, res: Response) {
  try {
    ...
    const result = await activityService.updateActivity(id, parsed.data, req.user!.id);

    if ("error" in result) {
      if (result.error === "NOT_FOUND") return res.status(404).json({ message: "Activity not found" });
      if (result.error === "FORBIDDEN") return res.status(403).json({ message: "You can only edit your own activities" });
    }

    return res.json({ message: "Activity updated successfully", activity: (result as any).activity });
  } catch (error) {
...
export async function remove(req: AuthRequest, res: Response) {
  try {
    ...
    if ("error" in result) {
      if (result.error === "NOT_FOUND") return res.status(404).json({ message: "Activity not found" });
      if (result.error === "FORBIDDEN") return res.status(403).json({ message: "You can only delete your own activities" });
    }
